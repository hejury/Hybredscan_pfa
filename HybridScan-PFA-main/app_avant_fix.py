...
_pages = [("Analysis","&#9673;"),("Requests","&#9636;"),("Quarantine","&#9632;"),("Folder Scan","&#9638;"),("Protection","&#9889;")]
if "pg" not in st.session_state: st.session_state.pg = 0
with st.sidebar:
    st.markdown('<div class="kl"><div class="kl-hex">&#11042;</div><div><div class="kl-t">Malware Detection</div><div class="kl-s">Threat Intelligence Portal</div></div></div>', unsafe_allow_html=True)
    for _i,(_n,_ic) in enumerate(_pages):
        _c = "ki actif" if _i==st.session_state.pg else "ki"
        st.markdown(f'<div class="{_c}">{_ic}&nbsp;&nbsp;{_n}</div>', unsafe_allow_html=True)
        if st.button(_n, key=f"kp{_i}", use_container_width=True):
            st.session_state.pg = _i; st.rerun()
    st.markdown('<div class="kf">Projet de fin d\'annee &middot; 2025/2026</div>', unsafe_allow_html=True)
def _pan(idx):
    return st.container()
def _actif(idx):
    return st.session_state.pg == idx
onglet1=_pan(0); onglet2=_pan(1); onglet3=_pan(2); onglet4=_pan(3); onglet5=_pan(4)
...
# ============ ONGLET 4 — SCAN DE DOSSIER ============
with onglet4:
    st.markdown("### Analyse d'un dossier complet")
    st.caption("Seuls les fichiers PE (.exe, .dll) sont analysés. "
    ...
    if st.button("Lancer le scan"):
        if not os.path.isdir(dossier):
            ...
            if not fichiers:
                st.warning("Aucun fichier PE trouvé dans ce dossier.")
            else:
                st.info(f"{len(fichiers)} fichier(s) PE détecté(s).")
                barre = st.progress(0)
                zone = st.empty()
                lignes, nb_mal = [], 0
                ...
                st.markdown("#### Résumé")
                a, b, c = st.columns(3)
                a.metric("Analysés", len(lignes))
                b.metric("Malveillants", nb_mal)
                c.metric("Sains", len(lignes) - nb_mal)

                if nb_mal:
                    st.error(f"{nb_mal} fichier(s) malveillant(s) détecté(s).")
                else:
...
# ============ ONGLET 5 — PROTECTION EN TEMPS RÉEL ============
with onglet5:
    ...
    st.markdown("### Protection en temps réel")
    st.caption("Tout nouveau fichier PE déposé dans les dossiers surveillés "
               "est analysé automatiquement.")

    if prot.active:
        st.success("Protection ACTIVÉE — surveillance en cours")
    else:
        st.warning("Protection DÉSACTIVÉE")
    ...
    st.markdown("#### Dossiers surveillés")
    for d in DOSSIERS:
        etat = "✓" if os.path.isdir(d) else "✗ (introuvable)"
        st.text(f"  {etat}  {d}")
    ...
    if not entrees:
        ...
        a, b = st.columns(2)
        a.metric("Fichiers analysés", len(entrees))
        b.metric("Malveillants", nb_mal)
