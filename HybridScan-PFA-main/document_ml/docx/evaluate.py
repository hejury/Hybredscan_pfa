#!/usr/bin/env python3
"""evaluate.py — Re-evaluation independante d'un model_docx.pkl deja entraine.

Usage :
    python3 evaluate.py --model model_docx.pkl --data test_set.csv --label-col label
...
que train.py, sans jamais reentrainer le modele. Sert a verifier qu'un
modele deploye n'a pas derive, ou a evaluer sur un dataset de test
...
l'avertissement affiche dans ce cas.

Aucun model_docx.pkl n'existe a ce stade (phase de fondation uniquement,
voir validation/DOCX-DATASET-PLAN.md) : ce script est une infrastructure
prete a l'emploi pour une phase future, pas une operation executee ici."""
import argparse
import json
import pickle
...
def main():
    ...
    ap.add_argument("--model", required=True, help="Chemin du bundle model_docx.pkl a evaluer")
    ap.add_argument("--data", required=True, help="CSV de test (colonnes = schema.FEATURES + label)")
    ap.add_argument("--label-col", default="label")
    args = ap.parse_args()

    # Meme pattern etabli pour PE (analyze.py::etape2_ia charge model.pkl)
    # et PDF (document_ml/pdf/predict.py) : --model est un chemin fourni
    # explicitement par l'operateur sur la ligne de commande d'un script
    # d'evaluation local, pas une entree televersee par un utilisateur via
    # l'application web. pickle reste le format de persistance standard
    # pour un estimateur scikit-learn ; aucune alternative JSON-native
    # n'existe pour un objet de ce type.
    ...
    if list(bundle["features"]) != list(FEATURES):
        print("AVERTISSEMENT : le schema de caracteristiques du modele charge ne correspond pas "
              "au schema courant de document_ml/docx/schema.py — resultats potentiellement invalides.")
